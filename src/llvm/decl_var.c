...
void llvm_decl_var(llvm_ctx *ctx, tree *typed_identifier) {
	...
	typed_identifier->av = llvm_acai_value_new_alloca_with_type(ctx, str, AST_CHILD_LEFT(typed_identifier)->v.i);
}
...
void llvm_decl_var_assigndeclare(llvm_ctx *ctx, tree *identifier, tree *node_literal) {
	...
	node_literal->av = llvm_acai_value_new_alloca(ctx, str);
}
